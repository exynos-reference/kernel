...
static int kbase_platform_power_clock_init(kbase_device *kbdev)
{
	...
	timeout = 10;
	while((__raw_readl(pmu_base_addr + EXYNOS5_G3D_STATUS) & 0x7) != 0x7) {
		if(timeout == 0) {
			/* need to call panic  */
...
{
	return ((__raw_readl(pmu_base_addr + EXYNOS5_G3D_STATUS) & 0x7) == 0x7) ? 1 : 0;
}
...
static int kbase_platform_power_off(void)
{
	...
	while(__raw_readl(pmu_base_addr + EXYNOS5_G3D_STATUS) & 0x7) {
		if(timeout == 0) {
			/* need to call panic */
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	do {
		tmp = __raw_readl(pmu_base_addr + EXYNOS5_CLKDIV_STAT_TOP0);
	} while (tmp & 0x1000000);
#endif /* CONFIG_MALI_T6XX_DVFS */
	return count;
}
...
static void set_clkout_for_3d(void)
{
	...
	__raw_writel(tmp, pmu_base_addr + /*S5P_PMU_DEBUG*/0x0A00);
...
int kbase_platform_regulator_init(void)
{
	...
	{
		kbase_platform_regulator_disable();
		printk("[kbase_platform_regulator_init] failed to set mali t6xx operating voltage [%d], ret %d\n", mali_gpu_vol, ret);
		//return -1;
	}
#endif /* CONFIG_REGULATOR */
